    ...
    def __init__(
        ...
        up_blocks: list[Layer] = [
            Block(  # UP BLOCK 1
                receive=(0, 2),
                layers=[
                    DDPMUpSample(previous_C=levels[2], receive=(0, 1)),  # (256 x H/2 x W/2)
                    Duplicate(factor=2, receive=(1,)),
                    DDPMResBlock(C=levels[2], receive=(0, 1)),  # (128 x H/2 x W/2)
                    DDPMResBlock(receive=(0, 1)),
                ],
            ),
            Block(  # UP BLOCK 2
                receive=(0, 2),
                layers=[
                    DDPMUpSample(previous_C=levels[1], receive=(0, 1)),  # (128 x H x W)
                    Duplicate(factor=2, receive=(1,)),
                    DDPMResBlock(C=levels[1], receive=(0, 1)),  # (64 x H x W)
                    DDPMResBlock(receive=(0, 1)),
                ],
            ),
        ]
