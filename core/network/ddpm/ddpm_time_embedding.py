...
class DDPMTimeEmbedding(Block):
    def __init__(self: DDPMTimeEmbedding, receive: Receive1 = (0,)) -> None:
        """
        0: time t (1 x 1) -> | 0: time embedding (512 x 1)
        """
        layers: list[Layer] = [
            SinEmbedding(dim=512, receive=(2,)),
            Linear(neuron_number=2048, receive=(2,)),
            SiLU(receive=(2,)),
            Linear(neuron_number=512, receive=(2,)),
        ]
