...
class Diffusion10M(Diffusion):
    def __init__(
        ...
        embeddings: list[Layer] = [
            ...
            # Image Embedding
            Conv(N=levels[1], K=3, receive=(0,)),  # (64 x H x W)
        ...
        down_blocks: list[Layer] = [
            Block(  # DOWN BLOCK 1
                receive=(0, 2),
                layers=[
                    Conv(N=levels[2], K=3, S=2, P=1),  # (128 x H/2 x W/2)
            ...
            Block(  # DOWN BLOCK 2 (BOTTLENECK)
                receive=(0, 1, 2),
                layers=[
                    Conv(N=levels[3], K=3, S=2, P=1),  # (256 x H/4 x W/4)
        ...
        up_blocks: list[Layer] = [
            Block(  # UP BLOCK 1
                receive=(0, 2),
                layers=[
                    DiffusionUpSample(previous_C=levels[2], receive=(0, 1)),  # (256 x H/2 x W/2)
                    Duplicate(factor=2, receive=(1,)),
                    DiffusionResBlock(C=levels[2], receive=(0, 1)),  # (128 x H/2 x W/2)
                    DiffusionResBlock(receive=(0, 1)),
                ],
            ),
            Block(  # UP BLOCK 2
                receive=(0, 2),
                layers=[
                    DiffusionUpSample(previous_C=levels[1], receive=(0, 1)),  # (128 x H x W)
                    Duplicate(factor=2, receive=(1,)),
                    DiffusionResBlock(C=levels[1], receive=(0, 1)),  # (64 x H x W)
                    DiffusionResBlock(receive=(0, 1)),
                ],
            ),
        ]
